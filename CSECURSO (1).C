...
else
{
 ...
 switch(key)
 {
  ...
  while(inkey!=283)
  {
     ...
     printHeading();
     colHeading();
for(ind = sal_begin, ctr = 0; ctr < start; ctr++, ind=data[ind].sal_next);
     for(ctr = 0; ctr < 10; ctr++,ind=data[ind].sal_next)
       showInfo(ctr,ind);

  while(inkey=bioskey(1) == 0);
    inkey = bioskey(0);
    gotoxy(1,1);
    switch(inkey)
    {
    ...
    if(start<0) { start=0; last = 9;}
     if(start>50) start=last=50;
     for(ind = sal_begin, ctr = 0; ctr < start; ctr++, ind=data[ind].sal_next);
     for(ctr = 0; ctr < 10; ctr++,ind=data[ind].sal_next)
       showInfo(ctr,ind);
  }    // till user presses ESC
  break;
  case 15616:
	clrscr();
